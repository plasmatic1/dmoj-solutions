...
const int MN = 2001, MNT = MN * MN;
int n, lineSz = 0,
    dp[MNT], bestEnd[MN]; // node, number of visited -> Max Last distance
pt pts[MN];
line lines[MNT];
...
int main(){
    ...
    Forinci(0, n) {
        Forincj(0, n) {
            if (i ^ j)
                lines[lineSz++] = {i, j, pts[i].dist(pts[j])};
        }
    }
    sort(lines, lines + lineSz);

    fill(bestEnd, bestEnd + n + 1, -INF);
    bestEnd[0] = 0;

    int lastDiff = INT_MAX, lastDiffId = 0; // Last time there was a diff. length line, and the associated id
    // Prevents you from doing A ---[6]---> B ---[6]---> C  
    Fori(0, lineSz) {
        // Lines shouldn't end at (0,0) (tho starting at origin is fine)
        if (!lines[i].b) continue;

        if (lines[i].d ^ lastDiff) {
            // debug("switch", lastDiffId, i);
            Forj(lastDiffId, i) {
                maxa(bestEnd[lines[j].b], dp[j]);
            }
        ...
        dp[i] = bestEnd[lines[i].a] + 1;
        // debug(i, lines[i].a, lines[i].b, lines[i].d, dp[i]);
    }

    println(*max_element(dp, dp + lineSz));
