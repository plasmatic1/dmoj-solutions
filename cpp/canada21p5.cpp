// ./canada-day-contest-2021-0-1.yml
#include <bits/stdc++.h>
using namespace std;
...
int main() {
    ...
    int mat = 0;
    for (auto i = 0; i < N; i++) mat += s[i] == t[i];

    Mat m(N+1, N+1);
    for (auto i = 0; i <= N; i++) {
        for (auto j = 0; j <= min(i, X); j++) {
            int right = i, wrong = N-i, cright = j, cwrong = X-j;
            if (cwrong > wrong) continue;

            int newright = i - cright + cwrong;
            MI pro = (C.choose(right, cright) * C.choose(wrong, cwrong)) / C.choose(N, X);
            m[newright][i] += pro;
        }
    }
