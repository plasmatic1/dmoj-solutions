...
const int MAX = 101, VMAX = 15, MV = (VMAX + 2) << 1, OFFS = VMAX, VMIN = -VMAX;
...
inline bool check(tr &cur, tr &adj){
	...
	if(dx == 0){
		int vy = dy / gcdv;
		for(int y = cur.y; y != adj.y; y += vy){
			if(bad[cur.x][y]) return true;
		}
	}
	else if(dy == 0){
		int vx = dx / gcdv;
		for(int x = cur.x; x != adj.x; x += vx){
			if(bad[x][cur.y]) return true;
		}
	}
	else{
