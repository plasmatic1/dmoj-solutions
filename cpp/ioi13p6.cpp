...
struct SegNode {
	...
	SegNode(int n) : l(nullptr), r(nullptr) {
		bst.insert(bst.root, new Node<int>(1, 0));
		bst.insert(bst.root, new Node<int>(n, 0));
	}  
};
struct SegmentTree {
...
#ifdef LOCAL
#define calcOp(...) calculate(__VA_ARGS__)
#define STR(...) #__VA_ARGS__
#define calc(p1, p2, p3, p4, ans) cout << STR(calcOp(p1, p2, p3, p4)) << "; (ans is " << #ans << ") :: " << calcOp(p1, p2, p3, p4) << endl; cout.flush()
