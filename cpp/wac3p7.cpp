...
const int MN = 1e5 + 1, MX = 1e6 + 1, TRIALS = 10;
...
bool run(int start) {
	...
	repi(0, n) {
		...
		if (oth != -1) {
			int sta = path[path.size() - 1], stb = path[path.size() - 2];

			setXY(sta, x, y); setDir(sta, stb, v, w);
			path.clear(); path.pb(sta);
			vis.reset(); vis[sta] = true;
			int foundIdx = -1;
			do {
				if (abs(v) >= MX || abs(w) >= MX) break;
				foundIdx = findAdj(x, y, v, w);
				if (foundIdx != -1) {
					if (vis[foundIdx]) break; // uh oh stinky! (2)
					apply(foundIdx, x, y, v, w, path);
				}
			} while (foundIdx != -1);
		}
