...
int n;
ll co[MN], fx[MN], fact[MN];

ll fpow(ll x, ll y) {
    if (y == 0LL) return 1LL;
    return (fpow((x * x) % MOD, y >> 1) * ((y & 1) ? x : 1LL)) % MOD;
}

inline ll divMod(ll x, ll y) { return (x * fpow(y, MOD - 2)) % MOD; }
inline ll choose(ll n, ll k) { return divMod(fact[n], (fact[k] * fact[n - k]) % MOD); }
inline void fixMod(ll &x) { x = ((x % MOD) + MOD) % MOD; }

inline ll getKthDifference(int k) {
    ll tot = 0, sign = 1;
    Forinci(0, k) {
        tot += (sign * choose(k, i) * fx[k - i]) % MOD;
        sign *= -1;
    }
    fixMod(tot);
...
int main(){
    ...
    for (int i = n; i >= 0; i--) {
        // Get dif
        ll dif = getKthDifference(i);
        co[i] = divMod(dif, fact[i]); // Using power rule
