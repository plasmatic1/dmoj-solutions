...
int main(){
    ...
    if (n == "0")
        ...
        Mat finalMul = {{1, 0}, {0, 1}}, pow = MUL;
        for (int i = n.length() - 1; i >= 0; i--) {
            int v = n[i] - '0';
            for (int j = 0; j < v; j++)
                finalMul = mul(finalMul, pow);

            Mat newPow = {{1, 0}, {0, 1}}; // Identity matrix
            for (int j = 0; j < 10; j++)
                newPow = mul(newPow, pow);
            pow = newPow;
        }

        Mat ans = mul(ST, finalMul);
        // debug(ans);
        println(ans[0][0]);
    }
