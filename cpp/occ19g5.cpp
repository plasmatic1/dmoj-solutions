...
const int MN = 1e3 + 1, MM = 5e3 + 1;
int n, m, k = 10,
    A[MM], B[MM], W[MM];
bool block[MN];

bool work(int maxEd, int cur, int cnt) {
    if (cur == m) return true;

    if (W[cur] > maxEd && !block[A[cur]] && !block[B[cur]]) {
        if (cnt >= k)
            return false;
        
        block[A[cur]] = true;
        bool ans = work(maxEd, cur + 1, cnt + 1);
        block[A[cur]] = false;
        if (ans) return true;

        block[B[cur]] = true;
        ans = work(maxEd, cur + 1, cnt + 1);
        block[B[cur]] = false;  
        return ans;
    }

    return work(maxEd, cur + 1, cnt);
}
