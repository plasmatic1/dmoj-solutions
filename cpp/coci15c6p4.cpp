...
const ll MOD = 1e9;
const int MN = 21, MP = 401, MSUB = 1 << 20;
int n;
ll dp[2][MSUB];
vpi pairs;
vi msks;
...
int main(){
    ...
    Forinci(1, n) {
        Forincj(i + 1, n) {
            if (gcd(i, j) == 1) {
                pairs.pb(mpr(i, j));
                int msk = 0;
                Forinck(i, j - 1) // skip last bit so that touch masks don't count as overlap (i.e. 1100 and 0011 turn into 1000 and 0010).  On the other hand, masks like 1110 and 0111 turn into overlapping (they turn into 1100 and 0110 respectively)
                    msk |= 1 << (k - 1);
                msks.pb(msk);
                // debug(i, j);
            }
    ...
    // dp
    int sz = pairs.size();
    int cur = 0, nxt = 1, msub = 1 << n;
    dp[cur][0] = 1;
    Fori(0, sz) {
        for(int j = msub - 1; j >= 0; j--) {
            dp[cur][j] %= MOD;
            dp[nxt][j] = dp[cur][j];
            dp[nxt][j | msks[i]] += dp[cur][j];
        }
