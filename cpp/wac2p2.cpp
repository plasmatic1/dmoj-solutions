...
map<int, ll> cnt;
vec<change> chs;
map<ll, vec<int>, greater<ll>> chsMp;
...
int main() {
    ...
    // Normal times
    Fori(0, sz - 1) {
        ll d = chs[i].d - chs[i + 1].d - 1;
        if (d > 0)
            cnt[chs[i].p] += d;
    }

    // Important times
    auto chv = vec<pair<ll, vec<int>>>(chsMp.begin(), chsMp.end());
    sz = chv.size();
    Fori(0, sz) {
        int best = *min_element(chv[i].second.begin(), chv[i].second.end());
        // debug(i, chv[i].first, best);
        if (i > 0)
            mina(best, chv[i - 1].second.back());
        // if (i>0) debug(chv[i - 1].second.back());
