...
string medianOf(int l, int r) {
    int sz = (r - l + 1), h = sz / 2;
    if (sz % 2) { // Odd size
        return to_string(arr[l + h]);
    }

    int sumv = arr[l + h - 1] + arr[l + h];
    if (sumv % 2) {
        return to_string(sumv / 2) + ".5";
    }
    else {
        return to_string(sumv / 2);
    }
}
...
int main(){
    ...
    int maxv = *max_element(arr + 1, arr + n + 1);
    int h = n / 2;
    string q1 = medianOf(1, h);
    string q2 = medianOf(1, n);
    string q3 = medianOf(n - h + 1, n);

    // println(minv, maxv, q1, q2, q3);
