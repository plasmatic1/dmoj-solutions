...
void unite(int x, int y) { dsu[rt(x)] = rt(y); } // x -> y
...
int main(){
    ...
    string tmp;
    scan(N, Q, tmp);
    repi(0, N) st[i + 1] = tmp[i] == '1';

    ppow[0] = 1LL;
    repi(1, N)
        ppow[i] = mmul(ppow[i - 1], 2);
    reverse(ppow, ppow + N);
    ...
    repi(1, N + 1) {
        if (st[i]) {
            unite(i, i + 1);
            cur = madd(cur, ppow[i - 1]);
            // dblb("has 1"); db(i); db(cur); dbln;
        }
    }
    ...
    while (Q--) {
        ...
        while (l <= r) {
            int nxt = rt(l + 1);
            cur = madd(cur, ppow[l - 1]);
            unite(l, nxt); 
            // dblb("change to 1"); db(l); db(nxt); db(cur); dbln;
            l = nxt;
        }
