...
namespace mpt {
    int ctr = -1, letC = 0,
        map[26];
    void reset() { memset(map, -1, sizeof map); letC = 0; ctr = -1; }
    int ulet(char c) { return c - 'A'; }
    char compress(char c) {
        if (map[ulet(c)] == -1) {
            letC++;
            return map[ulet(c)] = ++ctr;
        }
        return map[ulet(c)];
    }
} using mpt::compress; using mpt::letC;

inline int getVal(string &s, int dig) {
    if (dig >= s.length()) return 0;
    return valOf[(int)s[s.length() - dig - 1]];
}

inline bool check() {
    bool carry = false;

    int end = max(sl, max(tl, al));
    Fori(0, end) {
        int v = getVal(s, i) + getVal(t, i) + carry;
        if ((v % 10) ^ (getVal(a, i)))
            return false;
        carry = v >= 10;
    }
...
void rec(int letI) {
    if (letI == letC) {
        // debugArray(valOf, letC);
        found |= check();
        return;
    }

    Fori(letI == s[0] || letI == t[0] || letI == a[0], 10) {
        if (!used[i]) {
            valOf[letI] = i;
            used[i] = true;
...
void solve() {
    Fori(0, sl) s[i] = compress(s[i]);
    Fori(0, tl) t[i] = compress(t[i]);
    Fori(0, al) a[i] = compress(a[i]);

    // debug(letC);
    // for (char c : s) debug((int)c);

    rec(0);

    for (char c : s) print(valOf[(int)c]);
    print('\n');
    for (char c : t) print(valOf[(int)c]);
    print('\n');
    for (char c : a) print(valOf[(int)c]);
    print('\n');
}
